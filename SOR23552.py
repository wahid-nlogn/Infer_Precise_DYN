...
#def array2d(w, h):
def array2d(w:Int, h:Int)->Tuple(Int,Int,Dyn):
    d = array('d', [0.0]) * (w * h)
    #if data is not None:
    #    return setup(d,data)
    return (w, h, d)
...
def getitem(arr:Tuple(Int,Int,Dyn), x_y:Tuple(int,Dyn)):
    w,h,d = arr
    (x, y) = x_y
    return d[idx(arr, x, y)]

def setitem(arr:Tuple(Int,Int,Dyn), x_y:Tuple(Int,Int), val:float)->Tuple(Int,Int,Dyn):   
    (x, y) = x_y
    w, h, d = arr
    d[idx(arr, x, y)] = val
    return (w, h, d)
