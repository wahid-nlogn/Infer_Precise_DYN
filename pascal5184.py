...
import time
def pascal_upp(n:Int) -> List(List(Int)):
    ...
    return s
def pascal_low(n:Int) -> List(List(Int)):
    # transpose of pascal_upp(n)    
    return [list(x) for x in zip(*pascal_upp(n))]
def pascal_sym(n:Int) -> List(List(Int)):
...
def nextperm(a:List(Int))->Bool:
    ...
    if i == 0:
        ...
        a[i - 1], a[j] = a[j], a[i - 1]
        return True
def perm3(n:Int, flag:Bool) -> List(Dyn):
    if (flag):
        if (n < 1):
            return []
        z = range(n)
