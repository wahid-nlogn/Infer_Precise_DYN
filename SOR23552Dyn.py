...
#def array2d(w, h):
def array2d(w:Dyn, h:Dyn)->Tuple(Dyn,Dyn,Dyn):
    d = array('d', [0.0]) * (w * h)
    #if data is not None:
    #    return setup(d,data)
    return (w, h, d)

def idx(arr:Tuple(Dyn,Dyn,Dyn), x : Dyn,  y:Dyn)->Dyn:
    w,h,d = arr
...
def getitem(arr:Tuple(Dyn,Dyn,Dyn), x_y:Tuple(Dyn,Dyn)):
    w,h,d = arr
    (x, y) = x_y
    return d[idx(arr, x, y)]

def setitem(arr:Tuple(Dyn,Dyn,Dyn), x_y:Tuple(Dyn,Dyn), val:Dyn)->Tuple(Dyn,Dyn,Dyn):   
    (x, y) = x_y
    w, h, d = arr
    d[idx(arr, x, y)] = val
    return (w, h, d)

def SOR_execute(omega:Dyn, G:Tuple(Dyn,Dyn,Dyn), cycles:Dyn):
    w,h,d = G
...
def bench_SOR(loops:Dyn, n:Dyn, cycles:Dyn):
